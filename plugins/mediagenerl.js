...
ezra({ nomCom: "igdl", categorie: "VIPER-Download" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    if (igvid.data.data.data[0].type == 'video') {
      zk.sendMessage(dest, { video: { url: igvid.data.data.data[0].url }, caption: "ig video downloader powered by *☢️VIPER MD☢️*", gifPlayback: false }, { quoted: ms })
    }
    else {
      zk.sendMessage(dest, { image: { url: igvid.data.data.data[0].url }, caption: "ig image downloader powered by *☢️VIPER MD☢️*" })
    }
  ...
  async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      `;
          ...
          zk.sendMessage(dest, { video: { url: result.hd }, caption: 'facebook video downloader powered by *☢️VIPER MD☢️*' }, { quoted: ms });
...
ezra({
  ...
  async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      `;
          zk.sendMessage(dest, { image: { url: result.thumbnail }, caption: caption }, { quoted: ms });
          zk.sendMessage(dest, { video: { url: result.sd }, caption: 'facebook video downloader powered by *☢️VIPER MD☢️*' }, { quoted: ms });
