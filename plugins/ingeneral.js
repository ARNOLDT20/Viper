...
ezra({
  ...
  try {
    ...
    const data = response.data;
    const messageText = `
📖 ☢️VIPER MD X-FORCE☢️ HOLY BIBLE 📖
...
ezra({
  ...
  try {
    ...
    const replyText = "*Wait ☢️VIPER MD X-FORCE☢️ is getting your pair code ...*";
...
ezra({
  ...
  if (!githubUsername) {
    return repondre("Give me a valid GitHub username like: github ARNOLDT20 or STARBOYT20");
  }
...
> Powered by ☢️VIPER MD X-FORCE☢️
    `;
    repondre(texa);
  } catch (err) {
    console.error(err);
