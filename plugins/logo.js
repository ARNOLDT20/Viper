...
ezra({
  ...
  if (!arg || arg == "") {
    repondre("*__Exemple : * " + prefixe + "hacker VIPER");
    return;
  }
  try {
    ...
    repondre("* processing...*");
    await zk.sendMessage(origineMessage, { image: { url: anu.image }, caption: "* \t Logo made by VIPER MD XFORCE*" }, { quoted: ms });
  }
  catch (e) {
    repondre("🥵🥵 " + e);
  }
...
ezra({ nomCom: "dragonball", categorie: "VIPER-Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    const noArgMsg = `*_EXEMPLE *:  ${prefixe}dragonball VIPER`;
    ...
    await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by VIPER MD XFORCE*" }, { quoted: ms });
  }
  catch (e) {
    repondre("🥵🥵 " + e);
  }
...
ezra({ nomCom: "naruto", categorie: "VIPER-Logo", reaction: "👀" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg == '') {
      repondre("*_Exemple : * " + prefixe + "naruto VIPER");
      return;
    }
    ...
    await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by VIPER MD XFORCE*" }, { quoted: ms });
  }
  catch (e) {
    repondre("🥵🥵 " + e);
  }
...
ezra({ nomCom: "didong", categorie: "VIPER-Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
  let { arg, repondre, prefixe, ms } = commandeOptions;
  try {
    var titre = "\t* logo by VIPER_Md*";
    var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
    if (!arg || arg == "") {
      repondre(`*exemple :* ${prefixe}didong VIPER`)
      return;
    }
...
ezra({ nomCom: "neonlight", categorie: "VIPER-Logo", reaction: "💡" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    /*.then((data) => {*/
    zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by VIPER XFORCE' }, { quoted: ms });
...
ezra(
  {
    ...
    if (!arg || arg == "") {
      ...
      repondre(`  Exemple :* ${prefixe}boom VIPER`)
      return;
    }
    try {
      var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
      ...
      repondre("  processing ...")
      await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t *Logo by VIPER MD XFORCE*" })

      // await zk.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by VIPER-Md*"})
...
ezra({ nomCom: "purple", categorie: "VIPER-Logo", reaction: "🧳" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg == "") {
      repondre(prefixe + "purple VIPER"); return;
    }
    const lien = "https://en.ephoto360.com/purple-text-effect-online-100.html"
    var img = await mumaker.ephoto(lien, arg);
    repondre(" processing ...")
    await zk.sendMessage(dest, { image: { url: img.image }, caption: "      *Logo byVIPER MD XFORCE*" });
  } catch (e) { repondre(e) }
...
ezra({ nomCom: "gold", categorie: "VIPER-Logo", reaction: "🧚🏿‍♀️" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg == "") {
      repondre(prefixe + "gold VIPER-MD"); return;
    }
...
ezra({ nomCom: "arena", categorie: "VIPER-Logo", reaction: "🥵" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg == "") {
      repondre(prefixe + "arena VIPER-MD"); return;
    }
...
ezra({ nomCom: "incandescent", categorie: "VIPER-Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg == "") {
      repondre(prefixe + "incandescent VIPER-MD"); return;
    }
...
ezra({ nomCom: "gif×1", categorie: "VIPER-Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 VIPER-MD`);
      return;
    }
